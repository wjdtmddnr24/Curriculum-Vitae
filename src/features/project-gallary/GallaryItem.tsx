...
const GallaryItem = ({ title, year, description, imageURL, techs }: GallaryItemProps) => {
  return (
    <Link
      href="#"
      draggable={false}
      className="hover:bg-gray-100 dark:hover:bg-gray-700 -m-2 p-2 rounded-xl transition-colors duration-200"
    >
...
const ThumbnailHolder = ({ title, imageURL }: { imageURL: StaticImageData | string | undefined; title: string }) => {
  if (imageURL) {
    return (
      <div className="relative overflow-hidden rounded-3xl">
        <Image
          className="w-full aspect-[304/193] bg-slate-300 dark:bg-slate-500 object-cover"
          src={imageURL}
          alt="thmbnail"
          priority
          draggable={false}
  ...
  return (
    <div className="w-full aspect-[304/193] rounded-3xl relative overflow-hidden">
